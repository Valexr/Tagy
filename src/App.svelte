...
<script>
	...
	const clickHandler = (event) => {
		...
		// opening and closing 'about' section
			...
			event.target.className === 'openAboutBtn' ||
			event.target.className === 'closeAboutBtn'
		) {
	...
	const keyDownHandler = (event) => {
		// moving tile(s)
		if (
			event.target.classList.contains('responsive') &&
			(event.which === ENTER_KEY || event.which === SPACE_KEY)
		) {
			moveTiles(event.target.firstChild.textContent);
		}
		// closing about section
		else if (isAboutVisible && event.which === ESCAPE_KEY)
			toggleAboutVisibility();
	};
</script>
